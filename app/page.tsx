...
const projects = [
  { title: '송도 센트럴 비즈파크', category: '건축사업', location: '인천광역시 연수구', image: 'https://images.unsplash.com/photo-1487958449943-2429e8be8625?auto=format&fit=crop&w=1200&q=85' },
  { title: '세종 스마트시티 인프라', category: '토목사업', location: '세종특별자치시', image: 'https://images.unsplash.com/photo-1511818966892-d7d671e672a2?auto=format&fit=crop&w=1200&q=85' },
  { title: '마곡 리버사이드 오피스', category: '건축사업', location: '서울특별시 강서구', image: 'https://images.unsplash.com/photo-1497366811353-6870744d04b2?auto=format&fit=crop&w=1200&q=85' },
]
    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <header className="absolute inset-x-0 top-0 z-50 border-b border-white/15 text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10">
          <Link href="#top" className="flex items-center gap-3" aria-label="신덕종합건설 홈">
            <span className="flex h-10 w-10 items-center justify-center bg-primary font-sans text-xl font-black italic">S</span>
            <span className="font-sans text-xl font-bold tracking-tight">신덕종합건설<span className="text-secondary">.</span></span>
          </Link>
          ...
          <div className="hidden items-center gap-5 xl:flex"><span className="text-xs text-white/60">대표전화 02. 3456. 7890</span><Link href="#contact" className="bg-secondary px-5 py-3 text-sm font-bold text-white transition hover:bg-secondary/90">프로젝트 문의</Link></div>
        </div>
      </header>
      ...
      <section id="projects" className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-32"><div className="mb-14 flex items-end justify-between"><div><p className="section-kicker">FEATURED PROJECTS</p><h2 className="section-title">시간이 증명하는<br /><span className="text-secondary">우리의 작품.</span></h2></div><Link href="#contact" className="hidden border-b border-primary pb-2 text-sm font-bold md:block">전체 프로젝트 보기 ↗</Link></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{projects.map((project, index) => <article key={project.title} className={`group ${index === 0 ? 'md:col-span-2 lg:col-span-2' : ''}`}><div className={`relative overflow-hidden ${index === 0 ? 'h-[390px]' : 'h-[390px]'}`}><img src={project.image} alt={project.title} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-6 pt-24 text-white"><p className="text-xs font-semibold text-secondary">{project.category}</p><h3 className="mt-2 text-2xl font-bold">{project.title}</h3><p className="mt-1 text-sm text-white/70">{project.location}</p></div></div></article>)}</div></section>
      ...
      <section id="contact" className="bg-muted/60 px-6 py-24 lg:px-10 lg:py-28"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[1fr_0.9fr]"><div><p className="section-kicker">CONTACT US</p><h2 className="section-title">당신의 프로젝트를<br /><span className="text-secondary">들려주세요.</span></h2><p className="mt-6 max-w-md text-muted-foreground">새로운 가능성을 함께 만들어가겠습니다. 프로젝트 문의 및 제휴 문의를 남겨주시면 빠르게 답변드리겠습니다.</p><div className="mt-10 space-y-4 text-sm"><p><strong className="mr-8 text-primary">대표전화</strong> 02. 3456. 7890</p><p><strong className="mr-8 text-primary">이메일</strong> contact@hangyeol.co.kr</p><p><strong className="mr-8 text-primary">주소</strong> 서울특별시 강남구 테헤란로 123</p></div></div><form className="space-y-4 bg-card p-6 md:p-8" onSubmit={(event) => event.preventDefault()}><div className="grid gap-4 md:grid-cols-2"><input aria-label="이름" placeholder="이름" className="border-b border-border bg-transparent px-1 py-4 text-sm outline-none placeholder:text-muted-foreground focus:border-secondary" /><input aria-label="연락처" placeholder="연락처" className="border-b border-border bg-transparent px-1 py-4 text-sm outline-none placeholder:text-muted-foreground focus:border-secondary" /></div><input aria-label="이메일" placeholder="이메일" type="email" className="w-full border-b border-border bg-transparent px-1 py-4 text-sm outline-none placeholder:text-muted-foreground focus:border-secondary" /><textarea aria-label="문의 내용" placeholder="문의 내용을 입력해주세요" rows={5} className="w-full resize-none border-b border-border bg-transparent px-1 py-4 text-sm outline-none placeholder:text-muted-foreground focus:border-secondary" /><button type="submit" className="w-full bg-primary px-6 py-4 text-sm font-bold text-primary-foreground transition hover:bg-primary/90">문의 보내기 <span className="ml-5 text-secondary">→</span></button></form></div></section>

      <footer className="bg-[#10263f] px-6 py-12 text-white lg:px-10"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-10 md:flex-row"><div><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center bg-secondary font-sans text-lg font-black italic">S</span><span className="text-lg font-bold">신덕종합건설<span className="text-secondary">.</span></span></div><p className="mt-5 text-xs leading-6 text-white/45">서울특별시 강남구 테헤란로 123<br />대표이사 신형섭 · 사업자등록번호 123-45-67890</p></div><div className="flex flex-wrap gap-x-10 gap-y-3 text-sm text-white/65">{menuItems.map((item) => <Link href={item.href} key={item.label} className="hover:text-white">{item.label}</Link>)}</div></div><div className="mx-auto mt-10 flex max-w-7xl justify-between border-t border-white/10 pt-6 text-[11px] text-white/35"><span>© 2025 SINDEOK CONSTRUCTION. ALL RIGHTS RESERVED.</span><span>개인정보처리방침</span></div></footer>
    </main>
  )
}
